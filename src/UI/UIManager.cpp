...
void UIManager::DrawExampleSelection() {
    ...
    ImGui::SameLine(); Utils::HelpMarker("Changing the example resets the simulation and visualization.");

    // Add Reset button
    if (ImGui::Button("Reset Current Example")) {
        m_application.RequestExampleLoad(m_currentSelectedExampleId);
    }
    ImGui::SameLine(); Utils::HelpMarker("Reloads the currently selected example from scratch.");
}
...
void UIManager::DrawVectorVisualizationControls() {
    ...
    ImGui::SameLine(); Utils::HelpMarker("Updates differentials while dragging. Can be slow!");
    ...
    ImGui::SameLine(); Utils::HelpMarker("Updates gradients while dragging. Very slow! Requires Real-time Differentials.");
    ImGui::EndDisabled();
    ...
    ImGui::SameLine(); Utils::HelpMarker("Scales vector lengths logarithmically. Ignores 'Linear Scale'.");

    ImGui::Indent();
    ImGui::BeginDisabled(!m_config.Display.useLogScale); // Disable if log scale off
    visualsNeedUpdate |= ImGui::SliderFloat("Target Max Log Length", &m_config.Display.targetMaxLogScale, 0.1f, 5.0f, "%.1f");
    ImGui::EndDisabled();
    ImGui::Unindent();

    ImGui::BeginDisabled(m_config.Display.useLogScale); // Disable if log scale on
    visualsNeedUpdate |= ImGui::SliderFloat("Linear Vector Scale", &m_config.Display.differentialScale, 1e-9f, 1e-5f, "%.3e", ImGuiSliderFlags_Logarithmic); // Use log scale for slider itself
    ImGui::EndDisabled();
...
void UIManager::DrawTPEControls() {
    ...
    ImGui::SameLine(); Utils::HelpMarker("Tangent point energy exponent q.");
    pq_changed |= ImGui::InputDouble("p", &m_config.TPE.p, 0.1, 1.0, "%.1f");
    ImGui::SameLine(); Utils::HelpMarker("Tangent point energy exponent p.");
    ...
    ImGui::SameLine(); Utils::HelpMarker("Adaptivity parameter for far-field approximations.");
    mesh_params_changed |= ImGui::InputDouble("Intersection Theta", &m_config.TPE.intersection_theta, 100.0, 0.0, "%.1f");
    ImGui::SameLine(); Utils::HelpMarker("Adaptivity parameter for intersection checks.");
    mesh_params_changed |= ImGui::InputDouble("Far Field Sep.", &m_config.TPE.farFieldSeparation, 0.01, 0.0, "%.3f");
    ImGui::SameLine(); Utils::HelpMarker("Separation parameter for far-field blocks.");
    mesh_params_changed |= ImGui::InputDouble("Near Field Sep.", &m_config.TPE.nearFieldSeparation, 0.1, 0.0, "%.2f");
    ImGui::SameLine(); Utils::HelpMarker("Separation parameter for near-field blocks.");
    mesh_params_changed |= ImGui::InputDouble("Near Field Inter.", &m_config.TPE.nearFieldIntersection, 100.0, 0.0, "%.1f");
    ImGui::SameLine(); Utils::HelpMarker("Parameter for near-field intersection calculations.");
    mesh_params_changed |= ImGui::InputInt("Max Refinement", &m_config.TPE.maxRefinement);
    ImGui::SameLine(); Utils::HelpMarker("Maximum depth of adaptive refinement.");
    mesh_params_changed |= ImGui::InputInt("Cluster Split Threshold", &m_config.TPE.clusterSplitThreshold);
    ImGui::SameLine(); Utils::HelpMarker("Max cluster size before splitting.");
    mesh_params_changed |= ImGui::InputInt("Parallel Perc Depth", &m_config.TPE.parallelPercolationDepth);
    ImGui::SameLine(); Utils::HelpMarker("Depth for parallel tree traversal.");
    mesh_params_changed |= ImGui::InputInt("Thread Count", &m_config.TPE.threadCount);
    ImGui::SameLine(); Utils::HelpMarker("Number of threads for parallel processing.");
...
void UIManager::DrawActionControls() {
    ...
    ImGui::SameLine(); Utils::HelpMarker("Number of physics steps per button click.");

    if (ImGui::Button("Update Mesh")) {
        m_application.RequestPhysicsStep(m_config.Opt.nLoopIterations);
    }

    if (ImGui::Button("Print Energy")) { m_application.RequestPrintEnergy(); }
    ImGui::SameLine(); Utils::HelpMarker("Prints energy for simulation objects to console.");

    if (ImGui::Button("Show Differential")) {
        m_application.RequestCalculateAndShowDifferential();
    }
    ImGui::SameLine(); Utils::HelpMarker("Calculates and displays the energy differential vectors.");

     if (ImGui::Button("Show Gradient")) {
        m_application.RequestCalculateAndShowGradient();
    }
    ImGui::SameLine(); Utils::HelpMarker("Calculates and displays the energy gradient vectors (requires valid differential).");
}
...
void UIManager::DrawDebugControls() {
    ...
    ImGui::SameLine(); Utils::HelpMarker("Toggles the visualization of the combined obstacle meshes used by Repulsor.");

    ImGui::PushItemWidth(110);
    const char* verbosityLevels[] = { "Disabled (0)", "Info (1)", "Debug (2)" };
    int currentVerbosity = m_config.Debug.verbosity;
    if(currentVerbosity < 0) currentVerbosity = 0;
    if(currentVerbosity > 2) currentVerbosity = 2;
    ...
    ImGui::SameLine(); Utils::HelpMarker("Controls the amount of log output from Polyscope and the application.");
    ImGui::PopItemWidth();
    ...
    ImGui::SameLine(); Utils::HelpMarker("Creates a new mesh using the current coordinates stored inside the selected Repulsor mesh object.");
    ImGui::EndDisabled();

    if (ImGui::Button("Print Camera Position")) {
        glm::vec3 pos = polyscope::view::getCameraWorldPosition();
        polyscope::info("Camera Position: " + std::to_string(pos.x) + ", " + std::to_string(pos.y) + ", " + std::to_string(pos.z));
    }
    ImGui::SameLine(); Utils::HelpMarker("Prints the current camera position to the console.");
}
